...
export class EventParticipantController {
        ...
        try {
            ...
            if (!user) {
                return response.status(400).json({
                    status: 400,
                    message:
                        "Id do participante informado não foi encontrado, tente outro",
                    data: {},
                });
            }
            ...
            if (participantAlreadyRegistered.length > 0) {
                return response.status(409).json({
                    status: 409,
                    message: "Usuário já está cadastrado no evento selecionado",
                    data: {},
                });
            }
    ...
    async listByUserId(request: Request, response: Response) {
        ...
        if (!id) {
            return response.status(400).json({
                status: 400,
                message: "Informe um Id de participante",
                data: {},
            });
        }
