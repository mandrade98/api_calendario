...
export class EventController {
    async save(request: Request, response: Response) {
        ...
        try {
            ...
            return response.status(201).json({ status: 201, message: "Cadastro realizado com sucesso", data: event })
        } catch (error) {
    ...
    async delete(request: Request, response: Response) {
        ...
        if (!id) {
            return response.status(400).json({ status: 400, message: "Informe o id do evento", data: {} })
        }

        try {
            const eventExists = await prismaClient.event.findUnique({ where: { id: id } })

            if (!eventExists) {
                return response.status(400).json({ status: 400, message: "Id do evento informado não foi encontrado, tente outro", data: {} })
            }

            await prismaClient.event.delete({ where: { id: id } })

            return response.status(200).json({ status: 200, message: "Evento deletado com sucesso", data: {} })
    ...
    async update(request: Request, response: Response) {
        ...
        try {
            const eventExists = await prismaClient.event.findUnique({ where: { id } })
            ...
            return response.status(200).json({ status: 200, message: "Evento atualizado com sucesso", data: {} })
    ...
    async listById(request: Request, response: Response) {
        ...
        if (!id) {
            return response.status(400).json({ status: 400, message: "Informe um Id do evento", data: {} })
        }
        ...
        try {
            ...
            return response.status(200).json({ status: 200, message: "Consulta realizada com sucesso", data: event })
        } catch (error) {
            return response.status(500).json({ status: 500, message: "Algo não ocorreu bem por aqui", data: {} })
        }
    }
