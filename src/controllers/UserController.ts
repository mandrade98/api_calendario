...
export class UserController {
            ...
            if (error instanceof Prisma.PrismaClientKnownRequestError) {
                if (error.code === "P2002") {
                    return response.status(409).json({
                        status: 409,
                        message: "Esse e-mail já foi cadastrado, tente outro",
                        data: {},
                    });
                }
            }
    ...
    async delete(request: Request, response: Response) {
        ...
        if (!emailAddress) {
            return response
                .status(400)
                .json({ status: 400, message: "Informe um e-mail", data: {} });
        }
        ...
        try {
            ...
            if (!emailExists) {
                return response.status(400).json({
                    status: 400,
                    message: "E-mail informado não foi encontrado, tente outro",
                    data: {},
                });
            }
